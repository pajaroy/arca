#!/usr/bin/env python3
# fix_metadata.py - Script universal de corrección de metadatos para ALMA_RESIST

"""
📝 Script: fix_metadata.py
🔖 ID: SCRIPT_2025-06-05_01
...
MODULE_MARKER = "ALMA_RESIST"  # Nombre esperado del directorio raíz del módulo
SCRIPT_VERSION = "1.0"
ALMA_VERSION = "Centralesis v2.3"  # Versión por defecto del sistema
...
def find_module_root(start_path: Path) -> Path:
    """
    Busca recursivamente hacia arriba el directorio raíz del módulo ALMA_RESIST
    
    Args:
        start_path: Ruta inicial (ubicación del script)
    
    Returns:
        Path del directorio raíz del módulo
    
    Raises:
        FileNotFoundError: Si no se encuentra el directorio raíz
    """
    ...
    raise FileNotFoundError("No se encontró el directorio raíz del módulo ALMA_RESIST")
...
def calculate_content_hash(metadata: Dict, rest_content: str) -> str:
    ...
    Args:
        ...
        rest_content: Contenido del archivo después del bloque de metadatos
...
def normalize_metadata(metadata: Dict, file_path: Path, module_root: Path, counter: int) -> Tuple[Dict, Dict]:
    ...
    Args:
        ...
        file_path: Ruta al archivo
        module_root: Directorio raíz del módulo
        counter: Contador para generación de IDs
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Herramienta de normalización de metadatos ALMA_RESIST')
    ...
    try:
        ...
        # Encontrar raíz del módulo
        module_root = find_module_root(script_dir)
        print(f"📂 Módulo identificado: {module_root}")
            ...
            for file in files:
                ...
                if file_path.suffix.lower() in SUPPORTED_EXTENSIONS:
                    ...
                    if result['action'] == 'error':
                        ...
                        status = f"✨ Modificado ({len(result['changes'])} campos)"
                    else:
        ...
        mode = "SIMULACIÓN" if args.dry_run else "EJECUCIÓN"
        print(f"\n✅ {mode} COMPLETADA")
        print(f"• Archivos procesados: {processed}")
        print(f"• Registro de cambios: {log_path}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("✋ Error: Se requiere PyYAML. Instale con: pip install PyYAML")
        sys.exit(1)
