#!/usr/bin/env python3
# fix_metadata_v3.1.py - Script universal de corrección de metadatos para ALMA_RESIST

"""
📝 Script: fix_metadata_v3.1.py
🔖 ID: SCRIPT_2025-06-05_03
...
def detect_encoding(file_path: Path) -> str:
    """Detecta la codificación de un archivo usando chardet con muestreo inteligente."""
    try:
...
def read_file(file_path: Path) -> str:
    """Lee un archivo con detección de encoding y manejo robusto de errores."""
...
def load_ignore_patterns(root_path: Path) -> Set[str]:
    """Carga patrones de exclusión desde archivo .fix_metadata_ignore con sintaxis similar a .gitignore."""
    ignore_patterns = set()
...
def extract_metadata(content: str, file_extension: str) -> Tuple[Optional[Dict], str, bool]:
    """Extrae metadatos en múltiples formatos (YAML, TOML, comentarios)."""
    # Intento 1: Bloque YAML estándar (---)
    ...
    # Intento 3: Comentarios especiales en archivos de código
...
def calculate_content_hash(metadata: Dict, rest_content: str) -> str:
    ...
    # Serialización inteligente según formato detectado
...
def generate_id(tipo: str, file_path: Path, content_hash: str) -> str:
    """Genera un ID único usando tipo, fecha y fragmento de hash."""
...
def determine_file_type(file_path: Path, content: str) -> str:
    """Determina el tipo de archivo basado en múltiples criterios."""
    ...
    if ext == '.md':
        if 'changelog' in content_sample:
            return 'changelog'
        if any(x in content_sample for x in ['bitácora', 'registro', 'log:']):
            return 'bitacora'
        if any(x in content_sample for x in ['decisión', 'acuerdo', 'resolución']):
            return 'decision'
        return 'documento'
    elif ext in ['.py', '.sh']:
...
def validate_metadata_field(field: str, value: Any) -> bool:
    """Valida campos específicos de metadata."""
...
def normalize_metadata(metadata: Dict, file_path: Path, module_root: Path, content_hash: str) -> Tuple[Dict, Dict]:
    """Normaliza metadatos preservando valores válidos existentes."""
    changes = {}
    today = datetime.date.today().isoformat()
    ...
    # Campos con valores por defecto (solo si no existen o son inválidos)
    defaults = {
        'version': '1.0',
    ...
    for field, default in defaults.items():
        ...
        # Conservar valores válidos existentes
    ...
    # Actualizar fecha de actualización siempre
...
def process_file(file_path: Path, module_root: Path, dry_run: bool, log_file: Path) -> Dict:
    ...
    try:
        ...
        # Calcular hash del contenido actual para generación de ID
...
def print_summary(stats: Dict) -> None:
    ...
    print(f"\n{'='*60}")
    print(f"{COLOR_BLUE}📊 RESUMEN DEL PROCESO - fix_metadata v{SCRIPT_VERSION}{COLOR_RESET}")
    print(f"{'='*60}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=f'Herramienta de normalización de metadatos ALMA_RESIST v{SCRIPT_VERSION}',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        epilog="Ejemplos:\n"
               "  Modo normal: python fix_metadata_v3.1.py\n"
               "  Dry run: python fix_metadata_v3.1.py --dry-run\n"
               "  Directorio específico: python fix_metadata_v3.1.py --root-dir ~/ALMA_RESIST"
    )
    ...
    parser.add_argument('--verbose', action='store_true', help='Mostrar información detallada de depuración')
    ...
    try:
        ...
        logger.debug(f"Patrones de exclusión cargados: {len(ignore_patterns)}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"{COLOR_YELLOW}⚠️ Advertencia: El paquete 'chardet' no está instalado. Usando UTF-8 por defecto.{COLOR_RESET}")
        # Sobrescribir función para no usar chardet
        def detect_encoding(file_path: Path) -> str:
            return 'utf-8'
