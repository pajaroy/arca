...
# changelog: "se cambio la funcion harvest_stock ahora marca el stock por genetica"
# linked_to: "~/trece/doc/prompt/data_cli/data_cli.md"
# status: "stable"
# ---
import cmd
import csv
...
class TreceCLI(cmd.Cmd):
    intro = """
                13 Cannabis Club
    Terminal de Gestión de Datos - Versión 0.1.3
    ...
    def load_data(self):
        ...
        self.data = {}
        try:
            for root, dirs, files in os.walk(self.data_dir):  # ← Cambio aquí
                for filename in files:
                    if filename.endswith('.csv'):
                        name = filename[:-4]  # Remove .csv extension
                        filepath = os.path.join(root, filename)  # ← Y aquí
    ...
    def do_harvest_stock(self, arg):
        """Gestión de stock de cosechas por cosecha específica"""
        try:
            ...
            for detalle in self.data.get('harvest_detail', []):
                ...
                cosecha = next((h for h in self.data.get('harvest', []) 
                            if h['id'] == harvest_id), 
                            {'cut_date': 'Fecha desconocida', 'description': 'Sin descripción'})
                
                # Obtener nombre de genética
                genetic_name = next(
            ...
            for retiro in self.data.get('withdrawals', []):
                ...
                if harvest_id:  # Solo si el retiro está asociado a una cosecha
                    genetic_name = next(
                        (g['name'] for g in self.data.get('genetics', [])
                        if g['id'] == genetic_id), f"Genética {genetic_id}"
                    )
            ...
            for harvest_id, geneticas in stock_por_cosecha.items():
                # Obtener info de la cosecha
                cosecha = next((h for h in self.data.get('harvest', []) 
                            if h['id'] == harvest_id), 
                            {'cut_date': 'Fecha desconocida', 'description': 'Sin descripción'})
                
                print(f"\n\033[1;33mCosecha {harvest_id} - {cosecha['cut_date']}:\033[0m")
                print(f"Descripción: {cosecha['description']}")
                print("-" * 50)
                
                total_cosecha = 0
                for genetic_name, grams in geneticas.items():
                    if grams > 0:
                ...
                if total_cosecha > 0:
                    print(f"  \033[1;35mTotal cosecha: {total_cosecha:,.2f} gramos\033[0m")
            
            if not hay_stock:
                print("No hay stock disponible en cosechas")
            else:
                print(f"\n\033[1;32mSTOCK GENERAL: {total_general:,.2f} gramos\033[0m")
