#!/usr/bin/env python3
"""
Backup Manager 0.1.2
Corregido la parte de tomar la ruta para el archivo y create old backups, 
Se mejoro para qe no tome la ruta completa
...
class BackupManager:
    ...
    def _get_backup_filename(self, source_path: Path, description: str = None) -> str:
        ...
        # ✅ Usar solo el nombre del archivo/directorio
    ...
    def _get_backup_filepath(self, source_path: Path, description: str = None) -> Path:
        ...
        # ✅ Mantener la estructura de directorios completa
        backup_dir = self.backup_path / relative_path.parent
        
        # ✅ Pero usar solo el nombre del archivo para el nombre del backup
    ...
    def _cleanup_old_backups(self, source_path: Path, max_backups: int):
        ...
        # ✅ Usar solo el nombre del archivo para el patrón
        base_pattern = source_path.name.replace(' ', '_').replace('/', '_')
